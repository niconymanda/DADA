...
def custom_collate(batch):
    ...
    for sample in batch:
        ...
        positives.append(sample["positive"])
        negatives.append(sample["negative"])
...
class CommonVoiceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        filenames = [os.path.join(self.clip_dir, f) for f in filenames]
    ...
    def load_audio_tensor(self, filename):
        
        try:
            audio_arr, sr = load_audio(filename, self.sampling_rate)
        except Exception as e:
            print(f"Error loading audio file for idx {filename} from commonvoice")
            exit()
    ...
    def __getitem__(self, idx):
        x = self.load_audio_tensor(self.id_to_filename[idx])
        ...
        if self.mode == "classification":
            ...
            x_p = self.load_audio_tensor(self.id_to_filename[id_p])
            x_n = self.load_audio_tensor(self.id_to_filename[id_n])
...
class ASVSpoof19LADataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.id_to_filename = dict(enumerate([os.path.join(self.root_dir, f"flac/{key}.flac") for key in self.id_to_key.values()]))

        for idx, key in self.id_to_key.items():
            assert os.path.exists(os.path.join(self.root_dir, f"flac/{key}.flac")), f"File {key} does not exist"

        print(f"Loaded {len(self)} samples from ASVspoof19 LA {self.split}")

    def __len__(self):
        return len(self.id_to_key)
    
    def __repr__(self):
        return f'ASVSpoof19LADataset_{self.split}'

    def load_audio_tensor(self, filename):
        try:
            audio_arr, _ = load_audio(filename, self.sampling_rate)
        except Exception as e:
            print(f"Error loading audio file for {filename} from asvspoof19")
            exit()
            # return torch.zeros
        audio_tensor = torch.tensor(pad(audio_arr, self.cut)).float()
        return audio_tensor
    ...
    def get_triplets_from_anchor(self, anchor_idx):
        anchor_label = self.id_to_label[anchor_idx]
        positive_id = np.random.choice(
            np.where(self.df["label"].values == anchor_label)[0]
        )
        negative_ids = np.where(self.df["label"].values != anchor_label)[0]

        return positive_id, np.random.choice(negative_ids)

    def __getitem__(self, idx):
        key = self.id_to_key[idx]
        assert "LA" in key, f"Key {key} does not belong to ASVspoof19 LA dataset"
        label = self.id_to_label[idx]
        filename = self.id_to_filename[idx]
        transcription = self.id_to_transcription[idx]
        x = self.load_audio_tensor(filename)
