...
class TesterAuthorshipAttribution:
    ...
    def __init__(
        self, model, repository_id, author_id_map, classification_model=None, args=None
    ):
        self.model = model
        self.repository_id = repository_id
        self.author_id_map = author_id_map
        self.classification_model = (
            classification_model if classification_model else None
        )
        ...
        self.distance_function = config.get_distance_function(
    ...
    def test(self, test_dataloader, spoofed_dataloader):
        ...
        if self.classification_model is not None:
            if self.args.classification_head == "gmm":
                ...
                classif_results = self.gmm_predict(
                    self.classification_model, all_embeddings, all_labels
                )

                all_embeddings_spoofed, all_labels_spoofed = self.extract_embeddings(
                    spoofed_dataloader
                )
                classif_results_spoofed = self.gmm_predict(
                    self.classification_model,
                    all_embeddings_spoofed,
                    all_labels_spoofed,
                )
            else:
            ...
            config.write_results_to_file(
                results_spoofed, "./output/results_spoofed.txt", self.args
            )

        self.plot_tsne_for_authors(test_dataloader)
        config.save_model_config(
            self.args, output_path=f"{self.repository_id}/model_config.json"
        )
    ...
    def test_abx_accuracy(self, test_dataloader):
        ...
        with torch.no_grad():
            for batch in tqdm(test_dataloader):
                ...
                positive_attention_mask = batch["positive_attention_mask"].to(
                    self.device
                )
                ...
                cosine_similarity_pos = F.cosine_similarity(
                    anchor_embeddings, positive_embeddings
                )
                cosine_distance_pos = 1 - cosine_similarity_pos
                self.all_cosine_distances_positive.extend(
                    cosine_distance_pos.cpu().numpy()
                )
                ...
                for i in range(anchor_embeddings.size(0)):
                    ...
                    negative_attention_mask = negative_attention_masks_list[i, 0].to(
                        self.device
                    )
                    negative_embeddings = self.model(
                        negative_input_ids.unsqueeze(0),
                        negative_attention_mask.unsqueeze(0),
                    )
                    min_negative_embeddings = negative_embeddings

                    for neg_idx in range(
                        negative_inputs_ids_list.size(1)
                    ):  # N_negatives per anchor = #num_labels - 1
                        negative_input_ids = negative_inputs_ids_list[i, neg_idx].to(
                            self.device
                        )
                        negative_attention_mask = negative_attention_masks_list[
                            i, neg_idx
                        ].to(self.device)
                        negative_embeddings = self.model(
                            negative_input_ids.unsqueeze(0),
                        ...
                        dist_bx = self.distance_function(
                            anchor_embeddings[i].unsqueeze(0), negative_embeddings
                        )
                        if dist_bx < min_dist:
                            min_dist = dist_bx
                    ...
                    cosine_similarity_neg = F.cosine_similarity(
                        anchor_embeddings[i], min_negative_embeddings
                    )
                    cosine_distance_neg = 1 - cosine_similarity_neg
                    self.all_cosine_distances_negative.extend(
                        cosine_distance_neg.cpu().numpy()
                    )
                ...
                for i in range(anchor_embeddings.size(0)):
                    ...
                    for neg_idx in range(negative_inputs_ids_list.size(1)):
                        ...
                        if dist_bx < min_dist:
                            ...
                            min_negative_embeddings = negative_embeddings
                            print(
                                f"+: {dist_ax[i]}, _: {dist_bx}, anchor: {anchor_labels[i]}, negative: {negative_labels[i][neg_idx]}"
                            )
                            # print(f"Min dist: {min_dist}, possible label: {correct_label}")
                    cosine_similarity_neg = F.cosine_similarity(
                        anchor_embeddings[i], min_negative_embeddings
    ...
    def plot_tsne_for_authors(self, dataloader):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                embeddings = (
                    self.classification_model(input_ids, attention_mask).cpu().numpy()
                )
        ...
        tsne = TSNE(
            n_components=num_classes, perplexity=80, max_iter=3000, method="exact"
        )
    ...
    def plot_cosine_distence_distribution(self, test_dataloader):
        ...
        data = pd.DataFrame(
            {
                "Cosine Distance": np.concatenate(
                    [
                        self.all_cosine_distances_positive,
                        self.all_cosine_distances_negative,
                    ]
                ),
                "Pair Type": ["Anchor-Positive"]
                * len(self.all_cosine_distances_positive)
                + ["Anchor-Negative"] * len(self.all_cosine_distances_negative),
            }
        )
