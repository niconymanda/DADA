...
class SLIMStage1(nn.Module):
    ...
    """
    def __init__(self, style_layers=(0, 10), linguistics_layers = (14, 21), K=23, F_in=1024, F_out = 256, bottleneck_dropout=0.5):
        ...
        self.linguistics_compression = CompressionModule(K, F_in, F_out, bottleneck_dropout)
    ...
    def forward(self, x):
        ...
        out_linguistics = self.linguistics_model(x, output_hidden_states=True, return_dict=True)

        x_style = torch.cat(out_style.hidden_states[self.style_layers[0]:self.style_layers[1]], dim=-1)
        x_linguistics = torch.cat(out_linguistics.hidden_states[self.linguistics_layers[0]:self.linguistics_layers[1]], dim=-1)
