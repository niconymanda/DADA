% !TEX root = main.tex
% https://sites.stat.washington.edu/pds/stat423/Documents/Ehrenberg.reports.pdf for @abhaydmathur
...
% \subsection{General intro about dangers of deepfakes}
...
can be created with open-source software.\todo{citation needed} Distinguishing
...
still lacking.\todo{cite in the wild} This paper contributes to proactive
...
% \subsection{Why this isn't solved: ASVspoof vs In the wild}

Current automated deepfake detection methods typically use deep learning, and
achieve high benchmark performance \todo{citation needed, eg asvspoof}. But
deepfake detectors learn to spot acoustic artefacts produced by the generator
\todo{citation needed}, which is a major source of brittleness. Trained
deepfake detectors have poor performance when deployed in a real-world setting,
regardless of performance on standardized benchmarks \todo{cite in the wild}.
...
% \todo[inline]{Nils: add a description of benchmark datasets. In particular, describe the contrast between ASVspoof ("easy" task) and In the wild ("hard task"). You should clarify here that you are trying to solve the hard task.}
...
% \todo[inline]{Abhay: not sure if using citations independently is consistent/recommended.}
% 
...
% \todo[inline]{Abhay: Again, using citation  independently.} 
...
% \todo[inline]{Nils: reference Figures 2 and 3}
...
\todo[inline]{See Section \ref{sec:late-fusion} after the biblioraphy for a math description of late fusion}

% \todo[inline]{Abhay: I'll complete this today \today.}
...
% \todo[inline]{Abhay: Need to decide whether we group by task\{text, audio, fusion\} or by \{datasets,metrics,baselines\}. Further, should we call the text model the `context module'? BY TASK }
% \todo[inline]{Nils: my advice would be to group by task}
...
% \subsubsection{Detection Cost Function for Tandem Assessment (t-DCF)}
% The tandem assessment Detection Cost Function (t-DCF) is a metric used to
% evaluate the performance of spoofing countermeasures in conjunction with
% automatic speaker verification (ASV) systems~\cite{tdcf}. It combines the detection error
% trade-off (DET) curve of the ASV system with the performance of the spoofing
% countermeasure to provide a single metric that reflects the overall system
% performance. The t-DCF takes into account the costs of different types of
% errors and the prior probabilities of genuine and spoofed trials, making it a
% comprehensive metric for evaluating the effectiveness of anti-spoofing systems.
